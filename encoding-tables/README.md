...
- Optional: GPU command encodings, shader opcodes (see spec §5, §5.6)
...
**Conformance**: These encoding tables enable decoder and assembler conformance tests per NEXUS32_Specification_v1.0.md §2. The specification is authoritative; on any discrepancy between a table and the spec, the spec wins. Update the tables when the spec changes (with a corresponding CHANGELOG entry).
